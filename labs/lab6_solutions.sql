...
*/
SELECT
    'Cook books' AS category,
    COUNT(1) AS num_books,
...
SELECT DISTINCT
    ...
    reviews,
    price,
    AVG(price) OVER(PARTITION BY author) AS avg_price_per_author,
    (price - AVG(price) OVER(PARTITION BY author))*100.0/
    AVG(price) OVER(PARTITION BY author) AS pct_over_author_avg
FROM amazon_bestsellers
WHERE
    author IN (
        SELECT author
